...
app.post("/fetch_month", async (req, res) => {
  ...
  try {
    ...
    */
    ...
    let data = {};
    if (!summaryOnly) {
      const examplesResponse = await Example.find({ date: { $gte: startDate, $lt: endDate } }).populate('claimId', 'claimText'); // only pull in claimText from Claim
...
app.post("/fetch_next", async (req, res) => {
  const { dateString, summaryOnly, keywordId, claimId } = req.body.dateString;
  const baseDate = new Date(dateString);

  try {

    // Step 1: Find the next date that has any Example
    const nextExample = await Example.findOne(
      {
        date: { $gt: baseDate },
      ...
      null,
      { sort: { date: 1 } } // get earliest after given date
    );

    if (!nextExample) {
      return res.status(404).json({ message: "No records found after that date." });
    }

    // Step 2: Compute month range based on nextExample.date
    const nextDate = new Date(nextExample.date);
    ...
    const examples = await Example.find(matchQuery).populate('claimId', 'claimText');

    // Step 4: Build daily summary
